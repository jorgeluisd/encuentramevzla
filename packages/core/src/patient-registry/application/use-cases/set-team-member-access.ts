...
import { TeamAdminForbiddenError, TeamMemberNotFoundError } from "./team-admin-errors";

// Cambia el acceso de un miembro (rol/activo). El hospital_admin solo gestiona a su propio
// personal y no puede elevar a moderador global; el moderador, a cualquiera.
export class SetTeamMemberAccess {
  ...
  async execute(input: {
    ...
    await this.team.setAccess(input.memberId, input.changes);
  }
}
